...
ABC_SMC <- function(numParticles, species_fallout, taxa, esppres, n_traits,
  ...
  disp_vals <- vector("numeric", numParticles) #1:numParticles
  filt_vals <- vector("numeric", numParticles) #1:numParticles
  comp_vals <- vector("numeric", numParticles) #1:numParticles

  fits <- vector("numeric", numParticles) #1:numParticles
  rich_vec <- vector("numeric", numParticles) #1:numParticles
  eve_vec <- vector("numeric", numParticles) #1:numParticles
  div_vec <- vector("numeric", numParticles) #1:numParticles
  opt_vec <- vector("numeric", numParticles) #1:numParticles
  ...
  while (t < 50)  {
    ...
    # and reset
    next_weights <- rep(1,numParticles)
    next_disp <- vector("numeric", numParticles) #1:numParticles
    next_filt <- vector("numeric", numParticles) #1:numParticles
    next_comp <- vector("numeric", numParticles) #1:numParticles
