FD_dbFD_deprec <- function (x, a, w, w.abun = TRUE, stand.x = TRUE,
          ...
          calc.FDiv = TRUE, dist.bin = 2, print.pco = FALSE, messages = TRUE) 
{
    ...
    if (all(x.class2 == "factor" | x.class2 == "ordered")) {
      ...
      if (x.pco$nf > 1) {
        ...
        if (m < x.pco$nf) {
          ...
          if (!ade4::is.euclid(x.dist) & corr != "none") {
            qual.FRic <- sum(x.pco$eig[1:m])/sum(x.pco$eig)
            if (messages) 
          ...
          if (!ade4::is.euclid(x.dist) & corr == "none") {
            delta <- -0.5 * ade4::bicenter.wt(x.dist * x.dist)
        ...
        if (stand.FRic) {
          hull.all <- geometry::convhulln(traits.FRic, "FA")
  ...
  if (calc.FGR) {
    if (clust.type == "kmeans") {
      tr.clust <- vegan::cascadeKM(traits, km.inf.gr, km.sup.gr, 
                            km.iter, km.crit)
  ...
  if (is.matrix(x) | is.data.frame(x) & calc.CWM) {
    CWM <- FD_functcomp(x, a, CWM.type = CWM.type)
  }
